...
client.on("message", message => {
  ...
  let embed = new MessageEmbed().setColor("RANDOM").setFooter(`Pusha.`);


  if(komut === "ayarlar")  {
    let korumalar = Object.keys(chatkoruma).filter(k => k.includes('Guard'));
    if (!args[0] || !korumalar.some(k => k.includes(args[0]))) return message.channel.send(embed.setDescription(`Korumaları aktif etmek veya devre dışı bırakmak için **${config.prefix}ayar <koruma>** yazmanız yeterlidir! **Korumalar:** ${korumalar.map(k => `\`${k}\``).join(', ')}\n**Aktif Korumalar:** ${korumalar.filter(k => chatkoruma[k]).map(k => `\`${k}\``).join(', ')}`));
    let koruma = korumalar.find(k => k.includes(args[0]));
    chatkoruma[koruma] = !chatkoruma[koruma];
    fs.writeFile("./src/configs/koruma.json", JSON.stringify(chatkoruma), (err) => {
      if (err) console.log(err);
    });
    message.channel.send(embed.setDescription(`**${koruma}** koruması, ${message.author} tarafından ${chatkoruma[koruma] ? "aktif edildi" : "devre dışı bırakıldı"}!`));
  };
  ...
  function guvenli(kisiID) {
    ...
    client.on("message", async message => {
      if (message.author.bot || !message.guild) return;
      if (message.member.hasPermission('ADMINISTRATOR') || guvenli(message.member.id) || !chatkoruma.ReklamGuard) return;
...
if (newMessage.member.hasPermission('ADMINISTRATOR') || guvenli(newMessage.member.id)) return;
      ...
      if (pushakufur.some(pusha => ` ${newMessage.content.toLowerCase()} `.includes(` ${pusha} `))) {
...
if(message.member.roles.cache.get(config.mutedRole)) return;
   if (message.member.hasPermission('ADMINISTRATOR') || guvenli(message.member.id) || !chatkoruma.EtiketGuard) return;
...
client.on('message', async message => {
 if(message.author.bot || !message.guild) return;
 if(message.member.hasPermission('ADMINISTRATOR') || guvenli(message.member.id)|| !chatkoruma.SpamGuard) return;
 if(message.member.roles.cache.get(config.mutedRole)) return;
...
client.on('message', async message => {
  if(message.author.bot || !message.guild) return;
  if(message.member.hasPermission('ADMINISTRATOR') || guvenli(message.member.id) || !chatkoruma.UzunyazıGuard) return;
